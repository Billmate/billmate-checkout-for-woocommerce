...
class BCO_Request_Get_Payment extends BCO_Request {
	...
	public function request( $bco_order_number ) {
		...
		$request_args = apply_filters( 'bco_get_payment_args', $this->get_request_args( $bco_order_number ) );

		$response = wp_remote_request( $request_url, $request_args );
		$code     = wp_remote_retrieve_response_code( $response );

		// Log the request.
		$log = BCO_Logger::format_log( $bco_order_number, 'GET', 'BCO get payment', $request_args, json_decode( wp_remote_retrieve_body( $response ), true ), $code );
		BCO_Logger::log( $log );
